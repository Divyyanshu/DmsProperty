...
const getStatus = item =>
  safeStr(item.Status ?? item.status ?? item.EnquiryStatus ?? '')
    .toLowerCase()
    .trim();
...
const getPropertyIcon = (name = '') => {
  ...
  if (n.includes('sapphire'))
    return require('../../Assets/icons/stonestays1.png');
...
const buildBreakdown = enquiries => {
  ...
  enquiries.forEach(item => {
    ...
    if (!map[propName])
      map[propName] = { count: 0, pending: 0, converted: 0, new: 0 };
    map[propName].count++;
    if (status === 'pending') map[propName].pending++;
    else if (status === 'converted' || status === 'resolved')
      map[propName].converted++;
...
const PeriodDropdown = ({ selected, onSelect }) => {
  ...
  const selectedLabel =
    PERIOD_OPTIONS.find(o => o.key === selected)?.label ?? 'Monthly';
      ...
      >
        <View style={styles.chipCalIcon}>
          ...
          <View style={styles.chipCalDots}>
            {[0, 1, 2, 3, 4, 5].map(i => (
              <View key={i} style={styles.chipCalDot} />
            ))}
          </View>
        </View>
                  ...
                  return (
                    <View key={item.key}>
                      <TouchableOpacity
                        style={[
                          styles.ddOptRow,
                          isActive && styles.ddOptRowActive,
                        ]}
                        onPress={() => {
                          setOpen(false);
                          onSelect(item.key);
  ...
  <View style={{ gap: 12 }}>
    <View style={{ flexDirection: 'row', gap: 8 }}>
      {[0, 1, 2].map(i => (
        <View
          key={i}
          style={[styles.statCard, { flex: 1, padding: 14, gap: 6 }]}
        >
...
const EnquirySectionCard = ({
  ...
  <View style={styles.enqCard}>
    <View style={styles.enqCardHdr}>
      <View
        style={[styles.enqCardIconBox, { backgroundColor: categoryIconBg }]}
      >
        <Image
          source={categoryIcon}
          style={[styles.enqCardIcon, { tintColor: categoryIconTint }]}
          resizeMode="contain"
        />
      </View>
      <View style={{ flex: 1 }}>
        <Text style={styles.enqCardTitle}>{title}</Text>
        <Text style={styles.enqCardSub}>
          {properties.length} properties · {totalCount} enquiries
        </Text>
      </View>
      <View style={styles.totalBadge}>
...
const TotalEnquiryScreen = ({ navigation }) => {
  ...
  const searchedEnquiries = !search.trim()
    ...
    : filteredEnquiries.filter(
        i =>
          ...
          safeStr(i.FullName ?? i.fullName ?? i.name ?? '')
            .toLowerCase()
  ...
  const farmCount = searchedEnquiries.filter(
    i => (i.Category ?? i.category ?? '').toLowerCase() === 'farmhouse',
  ).length;
  const airbnbCount = searchedEnquiries.filter(
    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      >
          ...
          <View style={styles.statsRow}>
            <View style={styles.statCard}>
              ...
              <Text style={styles.statVal}>
                {enquiries.filter(i => getStatus(i) === 'new').length || total}
              </Text>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          onPress={() =>
            navigation.navigate('ReportsScreen', { activeTab: 'Enquiry' })
          }
        >
