...
const AmountInput = ({ value, onChangeText, placeholder, readOnly }) => {
  ...
  return (
    <View
      style={[
        inputSt.wrapper,
        focused && !readOnly && inputSt.wrapperFocused,
        amtSt.wrapperOverride,
        readOnly && amtSt.wrapperReadOnly,
      ]}
    >
...
const NewBookingScreen = ({ navigation, route }) => {
  ...
  const prefillForm = async id => {
    try {
      ...
      setBookingSource(
        BOOKING_SOURCE_OPTIONS.find(
          s => s.label.toLowerCase() === (d.BookingSource || '').toLowerCase(),
        ) || null,
      );
      setReceivedBy(
        RECEIVED_BY_OPTIONS.find(
          r => r.label.toLowerCase() === (d.ReceivedBy || '').toLowerCase(),
        ) || null,
      );
    } catch (err) {
  ...
  const submitLabel = isEditMode
    ? 'Update Booking'
    : catKey === 'farmhouse'
    ? 'Confirm Farmhouse Booking'
    : catKey === 'airbnb'
    ? 'Confirm Airbnb Booking'
    : 'Submit Booking';
        ...
        {formUnlocked && (
          <>
                  ...
                  {(dealAmount || advanceAmount) && (
                    <View style={balanceSt.hint}>
                      <Text style={balanceSt.hintText}>
                        ₹{parseFloat(dealAmount) || 0} Deal − ₹
                        {parseFloat(advanceAmount) || 0} Advance = ₹
                        {balanceAmount || 0}
                      </Text>
                    </View>
                  )}
            ...
            <TouchableOpacity
              style={[
                screenSt.submitBtn,
                isEditMode && screenSt.submitBtnEdit,
                loading && { opacity: 0.7 },
              ]}
              activeOpacity={0.85}
