...
const getPropertyIconConfig = (propertyName = '') => {
  ...
  if (name.includes('grey'))
    return {
      ...
      iconTint: Colors.primary_text,
    };
  if (name.includes('sky'))
    return {
      icon: require('../../Assets/icons/stonestays1.png'),
      iconBg: Colors.icon_cyan,
      iconTint: Colors.primary,
    };
  if (name.includes('topaz'))
    return {
      icon: require('../../Assets/icons/stonestays1.png'),
      iconBg: Colors.primary_light,
      iconTint: Colors.primary,
...
const PeriodDropdown = ({ selected, onSelect }) => {
  ...
  const selectedLabel =
    PERIOD_OPTIONS.find(o => o.key === selected)?.label ?? 'Today';
...
const ExpenseSectionCard = ({
  ...
  <View style={styles.expCard}>
    <View style={styles.expCardHdr}>
      <View
        style={[styles.expCardIconBox, { backgroundColor: categoryIconBg }]}
      >
        <Image
          source={categoryIcon}
...
const TotalExpensesScreen = ({ navigation }) => {
  ...
  const allFarmExp = expensesData.filter(
    i => i.Category?.toLowerCase() === 'farmhouse',
  );
  const allAirbnbExp = expensesData.filter(
    i => i.Category?.toLowerCase() === 'airbnb',
  );
  const farmTotal = allFarmExp.reduce(
    (s, i) => s + Number(i.ExpenseAmount || 0),
    0,
  );
  const airbnbTotal = allAirbnbExp.reduce(
    (s, i) => s + Number(i.ExpenseAmount || 0),
    0,
  );
        ...
        <View style={styles.summaryStrip}>
          <View style={[styles.stripItem, styles.stripItemBorder]}>
            ...
            <Text style={styles.stripVal}>
              {loading ? '...' : formatINR(overallTotal)}
            </Text>
            <Text style={styles.stripSub}>
              {allFarmExp.length + allAirbnbExp.length} expenses
            </Text>
          </View>
          <View style={[styles.stripItem, styles.stripItemBorder]}>
            <Text style={styles.stripLabel}>FARMHOUSE</Text>
            <Text style={styles.stripVal}>
              {loading ? '...' : formatINR(farmTotal)}
            </Text>
